      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8"> 
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
          {solutionsData.map((solution, index) => (
            <motion.div 
              key={index}
              className="bg-slate-50 p-6 md:p-8 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 ease-in-out transform hover:-translate-y-1" // Subtle hover lift
              custom={index}
              initial="hidden"
              whileInView="visible"
            ...
            >
              ...
              <div>
                <h3 className="text-xl md:text-2xl font-semibold mb-3 text-indigo-600">{solution.title}</h3>
                <p className="text-slate-600 leading-relaxed text-sm md:text-base">{solution.description}</p>
              </div>
            </motion.div>
          ))}
        </div>
